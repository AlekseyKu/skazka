...
- [x] Проверка: API поднимается, `/health` возвращает `status=ok`
...
- [x] Установить PostgreSQL (Docker)
- [x] Создать SQLAlchemy модели на основе текущей схемы SQLite
- [x] Настроить Alembic
- [x] Создать начальные миграции для всех таблиц
- [x] Применить миграции для создания структуры БД
- [ ] Данные из SQLite не переносим — только создание таблиц и полей
- [x] Протестировать подключение к PostgreSQL
- [x] Проверка: подключение к БД и простой запрос (SELECT 1)
...
- [x] DELETE /api/v1/collection/{id} - удаление из коллекции
- [x] POST /api/v1/collection/{id}/favorite - добавить в избранное
 - [x] Сначала read-only эндпоинты, затем операции изменения
...
- [x] GET /health - healthcheck endpoint
- [x] GET /health/ready - readiness probe (для Kubernetes/Docker)
...
- [x] Проверка: фронт открывается локально и рендерит стартовый экран

#### 3.2. Базовые компоненты
- [x] Создать Layout компонент
- [x] Создать базовые UI компоненты (Button, Card, Input)
- [ ] Настроить тему (цвета, типографика)
- [x] Создать хук useTelegram для работы с WebApp API
- [x] Настроить API client (axios/fetch)

#### 3.3. Аутентификация
- [x] Создать контекст аутентификации
- [x] Реализовать логин через Telegram
...
- [x] Форма генерации обычной сказки
- [ ] Форма именной сказки
- [ ] Форма сказки на ночь
- [x] Показ процесса генерации (loading)
- [x] Отображение результата (текст + аудио)
- [x] Обработка ошибок
